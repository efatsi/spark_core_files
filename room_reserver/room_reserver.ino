...
void loop() {
  if (waitingForFreeBusy) checkFreeBusyTimeout();

  if (receivedStatus) {
    if (!Spark.connected())      initializeConnection();
...
void handleRotation() {
  if (!receivedStatus || currentStatus == "busy" || minutes <= 5) return;
...
void initializeConnection() {
  ...
  // Spark.publish("mySpark/logger", "Connected!", 60, PRIVATE);
  Spark.subscribe(String("hook-response/graba/reserver_" + identifier), handleGrabaResponse, MY_DEVICES);
  pingFreeBusy();
}
...
void checkReserverTimeout() {
  if ((millis() - lastButtonPress) > 5000) {
    waitingForReserver = false;
    sadTimes();
  }
}
...
void checkFreeBusyTimeout() {
  if ((millis() - lastFreeBusyRequest) > 5000) {
    waitingForFreeBusy = false;
    pingFreeBusy();
  }
}

bool shouldPingFreeBusy() {
  return (mode == "showStatus" && (millis() - lastFreeBusyRequest) > 60000);
}

void handleButtonPress() {
  // Spark.publish("mySpark/logger", String("pinging for Reservation with duration: " + userInput), 60, PRIVATE);
  Spark.publish(String("graba/reserver_" + identifier), String("book:" + String(userInputInFives())), 60, PRIVATE);

  lastButtonPress = millis();
  waitingForReserver = true;
}

void pingFreeBusy() {
  // Spark.publish("mySpark/logger", "pinging for FreeBusy Status", 60, PRIVATE);
  Spark.publish(String("graba/reserver_" + identifier), "free_busy", 60, PRIVATE);

  lastFreeBusyRequest = millis();
  waitingForFreeBusy  = true;
}

void handleGrabaResponse(const char *name, const char *data) {
  ...
  String newStatus = response.substring(1, 5);

  // Spark.publish("mySpark/logger", String("Graba Reserver Response:" + response.substring(0, 20)), 60, PRIVATE);

  if (newStatus == "free" || newStatus == "busy") {
    if (waitingForReserver)         blankOutScreen();
    if (newStatus != currentStatus) resetScreen();
  ...
  if (waitingForReserver) waitingForReserver = false;
  if (waitingForFreeBusy) waitingForFreeBusy = false;
}
